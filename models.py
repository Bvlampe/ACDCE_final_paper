...
def models(cut_before_1990=False):
    ...
    for democ_diff in [1, 2]:
        for time_lag in [5, 10]:
            if democ_diff == 2 and time_lag == 10:
                continue
            print(f"Model: democratic score difference of at least {democ_diff}, time lag of {time_lag} years")
            ...
            # decrease = len(main_data[main_data["Democracy change"] > 0])
            # increase = len(main_data[main_data["Democracy change"] < 0])
            # stasis = len(main_data[main_data["Democracy change"] == 0])
            #
            # print("Decrease:", decrease, "or as %:", decrease / len(main_data))
            # print("Increase:", increase, "or as %:", increase / len(main_data))
            # print("Stasis", stasis, "or as %:", stasis / len(main_data))
            # print("Provisional total:", decrease + increase + stasis)

            main_data["Democracy decrease"] = np.where(main_data["Democracy change"] <= -democ_diff, True, False)
